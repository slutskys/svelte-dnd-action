 ...
 * @returns
 */
export function svelteNodeClone(el: HTMLElement) {
    const cloned = el.cloneNode(true);

    if (!(cloned instanceof HTMLElement)) {
        throw new Error("cloned HTMLElement did not return new HTMLElement");
    }

    if (el instanceof HTMLSelectElement && cloned instanceof HTMLSelectElement) {
        const values = [];
